...
const reservationHold = read("backend/src/config/reservationHold.js");
const reservationPaymentService = read("backend/src/services/reservationPaymentService.js");
const tableAvailabilityService = read("backend/src/services/tableAvailabilityService.js");
...
assert.match(reservationHold, /RESERVATION_PAYMENT_HOLD_HOURS = 3/);
assert.match(orderService, /heldUntil: reservationHoldDeadline\(\)/);
assert.match(reservationPaymentService, /heldUntil: reservationHoldDeadline\(\)/);
assert.match(reservationPaymentService, /status: "pending", heldUntil: mongoose\.trusted\(\{ \$gt: new Date\(\) \}\)/);
assert.doesNotMatch(tableAvailabilityService, /failedOrderIds/);
assert.match(tableAvailabilityService, /heldUntil: mongoose\.trusted\(\{ \$lte: new Date\(\) \}\)/);
...
assert.match(adminOrdersPage, /Payment controls the booking automatically/);
