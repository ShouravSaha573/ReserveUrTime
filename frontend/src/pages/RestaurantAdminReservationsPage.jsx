      ...
      <section className="mt-8 space-y-4">
        ...
        {reservations.map((reservation) => (
          <article key={reservation._id} className="surface rounded-3xl p-6 md:p-7">
            ...
            <div className="mt-5 grid gap-3 text-sm text-white/55 sm:grid-cols-2 lg:grid-cols-4"><p><span className="text-white/80">Date:</span> {reservation.reservationDate}</p><p><span className="text-white/80">Time:</span> {reservation.timeSlot}</p><p><span className="text-white/80">Guests:</span> {reservation.guestCount}</p><p><span className="text-white/80">Table:</span> {reservation.tableId?.tableNumber || "—"} · {reservation.tableId?.area || ""}</p></div>
