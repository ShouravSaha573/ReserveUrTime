...
function safeReturnTo(value) {
  if (
    value &&
    value.startsWith("/restaurant-admin/") &&
    !value.startsWith("//")
  ) {
    return value;
  }
...
export default function RestaurantAdminLoginPage() {
  ...
  async function submit(event) {
    ...
    try {
      const data = await apiFetch("/auth/restaurant-admin/login", {
        method: "POST",
        body: form,
        retryGet: false
      });
      ...
      <form onSubmit={submit} className="surface w-full rounded-[2rem] p-7 md:p-9">
        ...
        <div className="mt-8 space-y-5">
          <label className="block">
            <span className="mb-2 block text-sm text-white/60">Restaurant Admin email</span>
            <input
        ...
        <button className="btn-primary mt-7 w-full" disabled={state.loading}>
          {state.loading ? "Verifying..." : "Restaurant Admin login"}
        </button>
      </form>
    </main>
  );
